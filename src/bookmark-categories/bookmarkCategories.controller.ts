...
export class BookmarkCategoriesController {
  ...
  async findAll(@Query() bookmarkCategoryQueryDto: FindBookmarkCategoryDto ) {
    return await this.bookmarkCategoriesService.findAll(bookmarkCategoryQueryDto);
  }
...
//북마크 카테고리(그룹) 생성
  ...
  async updateBookmarkCategory(
    ...
    return await this.bookmarkCategoriesService.updateBookmarkCategory(Number(id), updateBookmarkCategoryDto);
  }
  ...
  async DeleteBookmarkCategory(
    @Param('category_id') id: number) {
      return await this.bookmarkCategoriesService.deleteBookmarkCategory(Number(id));
    }
}
